...
sealed trait Command {
  ...
  /** Executes the command.
    *
    * @return
    *   a message to show to the user in case of success (e.g. "Person added successfully!")
    */
  def execute(): AppOp[SuccessMsg]

  /** @return
    *   whether or not this is the exit command
    */
...
/** A command which adds an expense to the application state.
  *
  * It asks the user for the payer's name, the amount that was paid, and the list of participants.
  */
object AddExpenseCommand extends Command {
  ...
  def execute(): AppOp[SuccessMsg] = {
    ...
    /** TODO #26: Implement a function that reads participant names from console until the user enters END to finish.
      *
    ...
    /** TODO #28: Implement a function that finds a person by name.
      *
      * It should be a wrapper of the PersonService analogous function, handling translation between types and
      * converting None results to errors using the provided message (notice the return type is Person, and not
      * Option[Person]).
      *
...
case object AddPersonCommand extends Command {
  ...
  override def execute(): AppOp[SuccessMsg] = {
    ...
    /** TODO #29: Use readData() to read person data from console and use it to add a new person to the state.
      *
...
/** A command which computes the debt given all the people and expenses in the application state and prints a report to
  * console.
  *
