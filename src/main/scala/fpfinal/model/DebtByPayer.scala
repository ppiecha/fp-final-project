...
/** This class holds information about how much each person owns each particular payer.
  *
  * For example, suppose Bob owes Alice 10 dollars, and Charly owes Alice 20 dollars. In this case Alice would be the
  * payer, whereas Bob and Charly would be the payees.
  *
  * So if we want to know how much people owe to Alice, we can call debtForPayer(Alice) and get a DebtByPayee object
  * with this information. This object should contain an entry for Bob for 10 dollars, and an entry for Charly for 20
  * dollars.
  *
  * We can also query all the payers included in this object.
  *
  * @param debtByPerson
  *   a map containing each payer along with information about people who owe them (this information is in a DebtByPayee
  *   object)
  */
class DebtByPayer private (val debtByPerson: Map[Person, DebtByPayee]) {
  ...
  /** Return a simplified version of this DebtByPayer object which does not contain mutual debts.
    *
    * For example, if Alice owes Bob 30 dollars and Bob owes Alice 20 dollars, in the simplified version Alice owes Bob
    * 10 dollars.
    */
...
object DebtByPayer {
  ...
  /** Creates a DebtByPayer instance using the information contained in the map. Should be only used in tests.
    *
    * @param debtByPerson
    *   a map containing each payer along with how much people owe them
    */
  ...
  /** Creates a DebtByPayer from a single expense.
    *
    * The only payer is the payer from the expense. The payees are the participants in the expense, and each owes the
    * same amount (the total amount of the expense divided evenly among them).
    */
  ...
  /** TODO #20: Implement a monoid instance.
    *
    * Hint: Use the monoidMap instance and a suitable method to convert it to the instance you need.
    */
  implicit def monoidDebtByPayer(implicit
      monoidMap: Monoid[Map[Person, DebtByPayee]]
  ): Monoid[DebtByPayer] =
    new Monoid[DebtByPayer] {
      override def combine(x: DebtByPayer, y: DebtByPayer): DebtByPayer =
        DebtByPayer.unsafeCreate(monoidMap.combine(x.debtByPerson, y.debtByPerson))
      override def empty: DebtByPayer = DebtByPayer.unsafeCreate(Map.empty)
    }
