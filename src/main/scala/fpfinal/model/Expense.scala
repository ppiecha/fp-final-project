...
/** Represents a single expense.
  *
  * The participants are the people among who this expense will be divided. Note that this set does not contain the
  * payer.
  *
  * For example, assume Alice buys a pizza for 10 dollars to eat with Bob and Charly. In this case the expense
  ...
  *   the amount paid
  * @param participants
  *   the people involved in the expense (excluding the payer)
  */
class Expense private (
  ...
  /** TODO #11: Divide this amount among the payer and the participants so each has the same debt for this expense.
    *
...
object Expense {
  ...
  /** Creates an instance of a Expense without performing validations. Should only be used in tests.
    */
  def unsafeCreate(
      payer: Person,
      amount: Money,
  ...
  /** TODO #11b: Create a validated expense. The validations to perform are:
    ...
    *
    * Note: List's contains method will use == equality. Figure out a way to use the equality instance received in the
    * implicit argument eqPerson
    */
  def create(
      payer: Person,
      amount: Money,
  ...
  ): Eq[Expense] = new Eq[Expense] {
    override def eqv(x: Expense, y: Expense): Boolean =
      eqPerson.eqv(x.payer, y.payer) && eqMoney.eqv(
        x.amount,
        y.amount
      ) && eqParticipants.eqv(x.participants, y.participants)
  }
  ...
  implicit val showExpense: Show[Expense] = Show.show { e =>
    s"Expense[Payer=${e.payer.name},Amount=${e.amount.show},Participants=${e.participants.toList.mkString(",")}]"
  }
}
